...
var isWindows = ['win32', 'win64', 'win', 'windows'].indexOf(process.platform) >= 0;
...
function getCPUTemp(callback) {
    if (!isWindows) {
        require('child_process').exec("cat /sys/class/thermal/thermal_zone0/temp", function (err, stdout) {
            if (err != null) {
...
exports.usedmemPercentage = function () {
    return (_os.totalmem() - _os.freemem()) / _os.totalmem();
}
...
var percentageCPU = 0;
var temperatureCPU = 0;
exports.startStatGather = function (interval) {
    if (interval < 1000)
        interval = 1000;
    setInterval(function () {
        getCPUUsage(function (v) {
            percentageCPU = Math.round(v * 100);
        }, false);
        getCPUTemp(function (v) {
            temperatureCPU = Math.round(v);
        });
    }, interval);
}

exports.cpuUsageLast = function () {
    return percentageCPU;
}

exports.cpuTempLast = function () {
